...
- If a video stays black, this version now shows the actual YouTube IFrame Player error instead of hiding it.
- Errors **101/150** mean the uploader has disabled playback on embedded websites; SyncNest cannot override that YouTube restriction.
- Error **153** means YouTube did not receive the required referrer/client identity. This version explicitly uses `strict-origin-when-cross-origin` and sends the deployed origin to the player.
- Browser privacy/ad-block extensions or networks that block `youtube.com` can also prevent the iframe API from loading.
