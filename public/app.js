...
function youtubeErrorMessage(code) {
  if (code === 2) return ['Invalid YouTube video', 'The video ID is invalid. Paste a normal YouTube watch, shorts, live, or youtu.be link.'];
  if (code === 5) return ['YouTube playback failed', 'YouTube could not play this video in the HTML5 embedded player. Try another video or browser.'];
  if (code === 100) return ['Video unavailable', 'This YouTube video was removed, made private, or is otherwise unavailable.'];
  if (code === 101 || code === 150) return ['Embedding disabled', 'The video owner does not allow this video to play on other websites. Open it on YouTube or choose a different video.'];
  if (code === 153) return ['YouTube blocked the embed', 'YouTube did not receive the required site referrer. Reload the page; if it continues, check privacy/ad-block settings or the hosting referrer policy.'];
  return ['YouTube player error', `YouTube returned player error ${code || 'unknown'}. Try reloading or use another video.`];
}

function handleYouTubeError(event) {
  const code = Number(event?.data) || 0;
  youtubeErrorCode = code;
  loadedYoutubeId = '';
  applyingRemote = false;
  const [title, message] = youtubeErrorMessage(code);
  setEmptyState(title, message, true);
  showToast(title);
  console.warn('YouTube IFrame Player error:', code, message);
}

function handleYouTubeStateChange(event) {
  // CUED, BUFFERING, PLAYING, PAUSED and ENDED all confirm that the video loaded.
  if ([0, 1, 2, 3, 5].includes(Number(event?.data))) {
    youtubeErrorCode = 0;
    if (activeSource?.type === 'youtube') setEmptyState('', '', false);
  }
}

window.onYouTubeIframeAPIReady = function () {
  if (player || !window.YT?.Player) return;
  if (youtubeApiTimeout) clearTimeout(youtubeApiTimeout);
  ...
  player = new YT.Player('player', {
    ...
    events: {
      onReady: event => {
        youtubeReady = true;
        youtubeErrorCode = 0;
        // Explicitly preserve a cross-origin referrer for YouTube's player identity checks.
      ...
      onAutoplayBlocked: () => showToast('Click play once to allow YouTube playback')
    }
  });
};
...
function loadYouTubeIframeApi() {
  ...
  script.onerror = () => {
    if (activeSource?.type === 'youtube') {
      setEmptyState('YouTube API failed to load', 'Check the internet connection, ad/privacy blockers, or whether youtube.com is blocked on this network.', true);
    }
  };
  document.head.appendChild(script);

  youtubeApiTimeout = setTimeout(() => {
    if (!youtubeReady && activeSource?.type === 'youtube') {
      setEmptyState('YouTube is taking too long', 'The YouTube player API did not become ready. Reload the page or check browser/network blocking.', true);
    }
  }, 12000);
}
...
function cueYoutube(videoId) {
  ...
  applyingRemote = true;
  youtubeErrorCode = 0;
  setEmptyState('Loading YouTube…', 'Waiting for YouTube to cue the shared video.', true);
  try {
    ...
    applyingRemote = false;
    loadedYoutubeId = '';
    setEmptyState('Could not load YouTube', 'The YouTube player rejected this video. Reload the page or try another video.', true);
    console.error('cueVideoById failed:', error);
    return;
  }
  setTimeout(() => {
    applyingRemote = false;
    if (youtubeErrorCode) return;
...
function activePlayerReady() {
  ...
  if (activeSource.type === 'youtube') return youtubeReady && !youtubeErrorCode && loadedYoutubeId === activeSource.videoId;
